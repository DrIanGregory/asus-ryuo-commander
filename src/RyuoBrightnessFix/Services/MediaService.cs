...
public sealed class MediaService
{
    ...
    /// </summary>
    public async Task<(bool Ok, string Message)> SetPanelVideoAsync(
    ...
    private (bool Ok, string Message) SetPanelVideo(
        string sourceVideoPath, IProgress<string>? progress, CancellationToken ct)
    {
        try
        {
            if (!File.Exists(sourceVideoPath))
                return (false, "Source video not found: " + sourceVideoPath);

            var ffmpeg = FindFfmpeg();
            if (ffmpeg is null)
                return (false, "ffmpeg not found. Place ffmpeg.exe next to the app (see tools\\fetch-ffmpeg.ps1).");

            var adb = FindAdb();
            if (adb is null)
                return (false, "adb not found (install 'ASUS Info Hub - ROG RYUO IV').");
